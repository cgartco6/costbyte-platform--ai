...
router.post('/create-paypal-order', authenticate, async (req, res) => {
  ...
  request.requestBody({
    intent: 'CAPTURE',
    purchase_units: [{
      amount: { currency_code: 'ZAR', value: amount.toString() },
...
router.get('/execute-paypal', async (req, res) => {
  ...
  await db.run('INSERT INTO revenue (project_id, amount, currency, recorded_at) SELECT id, price, "ZAR", datetime("now") FROM projects WHERE id = ?', projectId);
...
router.post('/create-payfast', authenticate, async (req, res) => {
  ...
  const data = {
    ...
    item_name: description.substring(0, 100),
    m_payment_id: projectId
  };
  const pfOutput = querystring.stringify(data);
  const signature = crypto.createHash('md5').update(pfOutput + process.env.PAYFAST_PASSPHRASE).digest('hex');
...
router.post('/payfast-itn', async (req, res) => {
  ...
  if (payment_status === 'COMPLETE') {
    ...
    await db.run('INSERT INTO revenue (project_id, amount, currency, recorded_at) SELECT id, price, "ZAR", datetime("now") FROM projects WHERE id = ?', m_payment_id);
  }
  res.send('OK');
});

// Direct EFT notification (manual confirmation)
